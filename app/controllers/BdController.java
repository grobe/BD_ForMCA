...
public class BdController extends Controller {
	...
	public Result  scan(){
		play.Logger.debug("scan : MCA is HEre");
		  ...
		  return ws.url(url).get().thenApply(
	  	           response ->{
		                  // play.Logger.debug("FnacCrawler : crawler2 :thenApply :"+url+" "+ new Date());
		  	        	 play.Logger.debug("infoBD 1");
		  	        	//play.Logger.debug("response.getBody() : "+response.getBody());
		  	        	
		  	        	 BdData bdInfo = scanFromFnac.extractDataFromSearch( response.getBody(),IsbnCode) ;   
		  	        	 //ici chercher si ISBN deja existant si oui ne pas sauver.
		  	        	 bdInfo.save();
		  	        	play.Logger.debug("infoBD 2");
		  	        	return ok(views.html.bdInfo.render(bdInfo));
